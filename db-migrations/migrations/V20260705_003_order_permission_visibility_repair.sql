-- V20260705_003_order_permission_visibility_repair.sql
-- Purpose:
-- 1. Show the unified order list entry as an assignable page permission.
-- 2. Re-apply old sales/production order permission mappings for existing roles and user overrides.
-- 3. Keep old sales/production order permission nodes hidden after the unified order migration.
...
INSERT INTO sys_permission (parent_id, perm_code, perm_type, sort, perm_name, is_deleted)
SELECT parent.id, 'approval:order:audit', 3, 40, '订单审核', 0
FROM sys_permission parent
...
INSERT INTO sys_role_permission (role_id, permission_id, create_time, is_deleted)
SELECT DISTINCT role_item.id, permission.id, NOW(), 0
...
WHERE role_item.role_code IN ('ADMIN', 'TENANT_OWNER', 'SUPER_ADMIN', 'PLATFORM_ADMIN')
...
UPDATE sys_role_permission old_relation
INNER JOIN sys_permission old_permission ON old_permission.id = old_relation.permission_id
SET old_relation.is_deleted = 1
WHERE IFNULL(old_relation.is_deleted, 0) = 0
  AND old_permission.perm_code IN (
    'sales:order:*',
    'sales:order:add',
    'sales:order:list',
    'sales:order:detail',
...
UPDATE sys_user_permission old_override
INNER JOIN sys_permission old_permission ON old_permission.id = old_override.permission_id
SET old_override.is_deleted = 1,
    old_override.update_time = NOW()
WHERE IFNULL(old_override.is_deleted, 0) = 0
  AND old_permission.perm_code IN (
    'sales:order:*',
    'sales:order:add',
