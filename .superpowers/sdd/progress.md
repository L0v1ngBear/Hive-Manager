...
Task 11: implementation complete pending commit (management UI base/proxy switched from `/web` to `/api`; admin authentication namespaced under `/auth/admin`; order, approval, quality, and installation request modules use canonical routes; focused route tests 4/4, full Node suite 51/51, and Vite production build passed)
