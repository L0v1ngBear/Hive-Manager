...
Task 10: implementation complete pending review/commit (approved `db-migrations` tree imported byte-for-byte; 74 manifest files match 74 full SHA-256 entries; one top-level `scripts/migrate-db.sh` with required safety helpers; 28 retired application resource SQL files deleted; migration Node gate 2/2, schema-only baseline gate, and clean backend suite 184/184 passed)
