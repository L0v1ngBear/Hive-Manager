-- AI 经营建议权限升级脚本。
-- 说明：AI 全局洞察包含经营、客户、员工等高维分析，只应授权给老板、管理层或指定角色。
...
INSERT INTO sys_permission (parent_id, perm_code, perm_type, sort, perm_name, is_deleted)
SELECT parent.id, 'dashboard:ai:*', 2, 10, 'AI经营建议-全部权限', 0
...
INSERT INTO sys_permission (parent_id, perm_code, perm_type, sort, perm_name, is_deleted)
SELECT parent.id, 'dashboard:ai:view', 3, 1, '查看AI经营建议', 0
FROM sys_permission parent
...
FROM sys_permission parent
INNER JOIN (
    SELECT 'dashboard:ai:inventory' AS perm_code, 11 AS sort_no, 'AI建议-库存水位与周转' AS perm_name
    UNION ALL SELECT 'dashboard:ai:order', 12, 'AI建议-订单履约与交付'
    UNION ALL SELECT 'dashboard:ai:customer', 13, 'AI建议-客户客情与风控'
    UNION ALL SELECT 'dashboard:ai:quality', 14, 'AI建议-质量管控与溯源'
    UNION ALL SELECT 'dashboard:ai:finance', 15, 'AI建议-财务健康与成本'
    UNION ALL SELECT 'dashboard:ai:employee', 16, 'AI建议-员工组织与效率'
    UNION ALL SELECT 'dashboard:ai:operation', 17, 'AI建议-生产运营节奏'
) item
WHERE parent.perm_code = 'dashboard:ai:*'
  AND NOT EXISTS (SELECT 1 FROM sys_permission exists_permission WHERE exists_permission.perm_code = item.perm_code);

-- 已有租户负责人默认保留最高业务权限，避免升级后老板账号看不到 AI 建议。
