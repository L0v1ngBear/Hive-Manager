...
test('after-sales saves distinguish create, update, and assigned processing permissions', () => {
  ...
  assert.match(controller, /creating && !TenantPermissionContext\.hasPermission\(PermissionCatalogV3\.CODE_AFTER_SALES_CREATE\)/)
  assert.match(controller, /!creating && !TenantPermissionContext\.hasPermission\(PermissionCatalogV3\.CODE_AFTER_SALES_UPDATE\)\s*&& !TenantPermissionContext\.hasPermission\(PermissionCatalogV3\.CODE_AFTER_SALES_PROCESS\)/)
...
test('after-sales tickets can be assigned and notify the assigned employee', () => {
  ...
  assert.match(page, /function canEditTicket\(ticket = \{\}\) \{ return canUpdate\.value \|\| \(canProcess\.value && userStore\.userInfo\?\.userId != null && Number\(ticket\.assigneeUserId\) === Number\(userStore\.userInfo\.userId\)\) \}/)
...
test('all after-sales ticket statuses can revise data without reopening workflow or rebuilding outbound parts', () => {
  ...
  assert.match(service, /requireTicketEditAccess\(ticket, hasFullEditPermission\)/)
  assert.match(service, /仅被指派人可以补充该售后工单的处理资料/)
  assert.match(service, /editingWorkflowTicket = !creating && !STATUS_DRAFT\.equals\(ticket\.getStatus\(\)/)
  assert.match(service, /if \(!editingWorkflowTicket\) replaceTicketParts/)
  assert.match(service, /if \(!editingWorkflowTicket && Boolean\.TRUE\.equals\(request\.getApprovalRequired\(\)\)\) submitTicketApproval/)
...
test('motor replacement records both return-shipment directions and a registration date', () => {
  const migrationPath = 'db-migrations/migrations/V20260904_001_after_sales_motor_return_logistics_and_registration_date.sql'
  const migration = readRepo(migrationPath)

  assert.match(ticketEntity, /private LocalDate registrationDate;/)
  assert.match(ticketEntity, /private String manufacturerReturnLogisticsCompany;/)
  assert.match(ticketEntity, /private String manufacturerReturnWaybillNo;/)
  assert.match(ticketRequest, /private LocalDate registrationDate;/)
  assert.match(ticketRequest, /private String manufacturerReturnLogisticsCompany;/)
  assert.match(ticketRequest, /private String manufacturerReturnWaybillNo;/)
  assert.match(service, /ticket\.setRegistrationDate\(request\.getRegistrationDate\(\)\)/)
  assert.match(service, /ticket\.setManufacturerReturnLogisticsCompany\(clean\(request\.getManufacturerReturnLogisticsCompany\(\)\)\)/)
  assert.match(service, /ticket\.setManufacturerReturnWaybillNo\(clean\(request\.getManufacturerReturnWaybillNo\(\)\)\)/)
  assert.match(service, /"登记日期"/)
  assert.match(service, /"物流公司", "物流单号", "厂家返客户物流公司", "厂家返客户单号"/)
  assert.match(page, /label="登记日期"/)
  assert.match(page, /客户退回厂家物流公司/)
  assert.match(page, /厂家返客户物流公司/)
  assert.match(page, /detail\.manufacturerReturnWaybillNo/)
  assert.match(migration, /registration_date DATE NULL/)
  assert.match(migration, /manufacturer_return_logistics_company VARCHAR\(80\) NULL/)
  assert.match(migration, /manufacturer_return_waybill_no VARCHAR\(100\) NULL/)
  assert.match(manifest, /migrations\/V20260904_001_after_sales_motor_return_logistics_and_registration_date\.sql/)
  const hash = createHash('sha256').update(migration).digest('hex')
  assert.match(checksums, new RegExp(`${hash}  migrations/V20260904_001_after_sales_motor_return_logistics_and_registration_date\\.sql`))
})

test('intake creation defers processing details until the assignee edits the draft ticket', () => {
  assert.doesNotMatch(ticketRequest, /@NotBlank\(message = "请选择处理方式"\)\s*private String ticketType/)
...
test('assigned processors have a dedicated open-task tab with the processing method and next action', () => {
  ...
  assert.match(page, /<el-tab-pane v-if="canProcess" label="我的待办" name="my-tasks">/)
