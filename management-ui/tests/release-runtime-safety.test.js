...
test('root publish wrapper delegates to the guarded synchronization and restart entrypoints', () => {
  ...
  const directPublishStart = publish.indexOf('[ "${RELEASE_SOURCE_DIR}" != "${RELEASE_TARGET_DIR}" ] || {')
  assert.notEqual(directPublishStart, -1, 'publish wrapper must detect a single-directory release')
  const directPublishEnd = publish.indexOf('\n}\n', directPublishStart)
  assert.notEqual(directPublishEnd, -1, 'single-directory publish branch must be closed')
  const directPublish = publish.slice(directPublishStart, directPublishEnd + 3)
  assert.match(directPublish, /Publishing directly from runtime directory/)
  assert.match(directPublish, /cd "\$\{RELEASE_TARGET_DIR\}"[\s\S]*bash scripts\/restart\.sh[\s\S]*exit 0/)
  assert.doesNotMatch(directPublish, /sync-release-files\.sh/)
