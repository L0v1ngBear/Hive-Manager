...
test('employee login describes supported account identifiers and password reset', async () => {
  ...
  assert.match(source, />忘记密码？<\/a>/)
  assert.doesNotMatch(source, /首次登录\s*\/\s*忘记密码/)
...
test('employee creation presents one-time activation guidance from EmployeeCreateVO', async () => {
  ...
  assert.match(source, /登录页点击“忘记密码？”/)
  assert.doesNotMatch(source, /首次登录\s*\/\s*忘记密码/)
