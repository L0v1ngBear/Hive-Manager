...
test('migrates employee list and editor controls to Element Plus', () => {
  ...
  assert.match(employeePermissionDrawer, /<el-tree(?:\s|>)/)
  assert.match(employeePermissionDrawer, /<el-segmented(?:\s|>)/)
})
...
test('keeps employee commands visible but guards their real permission combinations', () => {
  ...
  assert.match(employeeList, /hasPermission\(['"]employee:permission:manage['"]\)/)
