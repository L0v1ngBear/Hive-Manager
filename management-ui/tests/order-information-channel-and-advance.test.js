...
test('order list replaces remark with all shipment delivery details', () => {
  ...
  assert.match(orderSource, /\.order-column-informationChannel\s*\{[\s\S]*?width:\s*10%/)
  assert.match(orderSource, /\.order-column-shipments\s*\{[\s\S]*?width:\s*15%/)
  assert.doesNotMatch(orderSource, /\.order-column-expressNo/)
  ...
  assert.match(orderSource, /\.order-list-table\.responsive-data-table\s*\{[\s\S]*?table-layout:\s*fixed/)
  assert.match(orderSource, /useLocalTableColumns\('order\.list\.commercial\.v5'/)
})
