<template>
  <div class="function-page-shell h-full min-h-0 font-body">
    <div class="function-page-container space-y-6">
      ...
      <el-result v-if="statsError" :icon="statsError.icon" :title="statsError.title" :sub-title="statsError.message"><template #extra><el-button @click="fetchStats">重试统计</el-button></template></el-result><section v-else v-loading="statsLoading" class="function-stats-grid grid grid-cols-1 gap-4 md:grid-cols-4"><el-statistic title="SKU 数量" :value="stats.skuCount" /><el-statistic title="平均基准价" :precision="2" :value="Number(stats.averagePrice ?? 0)" /><el-statistic title="计划中价格" :value="stats.pendingCount" /><el-statistic title="客户特价" :value="stats.overrideCount" /></section>
