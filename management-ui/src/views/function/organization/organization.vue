  ...
  <div class="function-page-shell h-full min-h-0 font-body">
    <div class="function-page-container space-y-6">
      ...
      <section class="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <article class="stat-card bg-amber-50 border-amber-100">
      ...
      <section class="grid grid-cols-1 xl:grid-cols-[minmax(0,1fr)_420px] gap-6">
        <div class="rounded-2xl bg-surface-container-lowest shadow-sm ring-1 ring-outline-variant/20 overflow-hidden">
          <div class="flex items-center justify-between border-b border-outline-variant/10 px-6 py-4">
            <div>
              ...
              <p class="mt-1 text-xs text-on-surface-variant">点击部门查看成员，右侧可维护部门信息。</p>
            </div>
            <button class="rounded-lg bg-surface-container-high px-4 py-2 text-xs font-bold text-on-surface" @click="fetchOverview">刷新</button>
          </div>
          ...
          <div class="relative min-h-[420px] p-5">
            ...
            <div v-else-if="!loading" class="flex min-h-[360px] flex-col items-center justify-center rounded-2xl bg-surface-container-low text-center">
              <span class="material-symbols-outlined text-primary text-5xl">account_tree</span>
              <p class="mt-3 text-sm font-bold text-on-surface">还没有部门</p>
              <p class="mt-1 text-xs text-on-surface-variant">先创建第一个部门，再把员工分配进来。</p>
              <button class="mt-4 rounded-xl bg-primary px-5 py-2 text-sm font-bold text-white" @click="openCreate(null)">新增部门</button>
            </div>
          </div>
        </div>

        <aside class="rounded-2xl bg-surface-container-lowest shadow-sm ring-1 ring-outline-variant/20 overflow-hidden">
          ...
          <div class="max-h-[560px] overflow-y-auto p-5 space-y-3">
            ...
            <div v-else-if="members.length" class="space-y-3">
              <div v-for="item in members" :key="item.id" class="rounded-xl border border-outline-variant/20 bg-surface-container-low p-4">
                <div class="flex items-start justify-between gap-3">
                  ...
                  <span :class="employeeStatusClass(item.status)" class="rounded-full px-2 py-0.5 text-[10px] font-bold">{{ employeeStatusLabel(item.status) }}</span>
                </div>
    ...
    <aside class="fixed top-0 right-0 z-50 flex h-full w-full max-w-[460px] flex-col border-l border-outline-variant/30 bg-white/95 shadow-2xl backdrop-blur-2xl transition-transform duration-300" :class="drawerVisible ? 'translate-x-0' : 'translate-x-full'">
      ...
      <div class="flex-1 space-y-5 overflow-y-auto p-6">
        <label class="block">
          ...
          <input v-model.trim="form.deptName" class="box-input" placeholder="例如：销售部、仓储部" />
        </label>
        <label class="block">
          <span class="field-label">部门编码</span>
          <input v-model.trim="form.deptCode" class="box-input" placeholder="不填则自动生成" />
        </label>
        <label class="block">
        ...
        <label class="block">
          ...
          <input v-model.trim="form.leaderName" class="box-input" placeholder="请输入负责人姓名" />
        </label>
    ...
    return () => h('div', { class: 'org-node-wrap' }, [
      h('div', {
        ...
        h('div', { class: 'flex min-w-0 flex-1 items-center gap-3' }, [
          ...
          h('div', { class: 'min-w-0' }, [
            h('p', { class: 'truncate text-sm font-black text-primary' }, props.node.deptName),
        ...
        h('div', { class: 'flex shrink-0 items-center gap-2' }, [
          h('span', { class: ['rounded-full px-2 py-0.5 text-[10px] font-bold', props.node.status === 1 ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-600'] }, props.node.status === 1 ? '启用' : '停用'),
...
const parentOptions = computed(() => flattenDepartments(departments.value))
...
async function fetchOverview() {
  ...
  try {
    ...
    if (!activeDepartment.value && departments.value.length) {
...
function flattenDepartments(nodes, level = 0) {
  return nodes.flatMap(node => [
    { id: node.id, label: `${'　'.repeat(level)}${node.deptName}` },
...
<style scoped>
.stat-card{border-radius:1rem;border:1px solid rgba(0,82,204,.1);background:rgb(var(--surface-container-lowest));padding:1.25rem;box-shadow:0 1px 2px rgba(15,23,42,.05)}
.stat-label{font-size:.7rem;font-weight:900;letter-spacing:.18em;text-transform:uppercase;color:rgb(var(--on-surface-variant))}
.stat-value{margin-top:.6rem;font-size:2rem;line-height:2.2rem;font-weight:900;color:rgb(var(--primary))}
.box-input{width:100%;border-radius:.75rem;border:1px solid rgba(148,163,184,.28);background:rgb(var(--surface-container-low));padding:.75rem 1rem;font-size:.875rem;outline:none}
.box-input:focus{border-color:rgb(var(--primary));box-shadow:0 0 0 3px rgba(0,82,204,.12)}
.field-label{margin-bottom:.5rem;display:block;font-size:.75rem;font-weight:900;letter-spacing:.08em;text-transform:uppercase;color:rgb(var(--primary))}
.org-node-wrap{position:relative}
.org-node{display:flex;align-items:center;justify-content:space-between;gap:1rem;border-radius:1rem;border:1px solid rgba(148,163,184,.18);background:white;padding:1rem;transition:all .2s ease}
.org-node:hover{border-color:rgba(0,82,204,.28);box-shadow:0 8px 24px rgba(15,23,42,.06);transform:translateY(-1px)}
.org-node.active{border-color:rgb(var(--primary));background:rgba(0,82,204,.05)}
.org-node-icon{display:flex;height:2.75rem;width:2.75rem;align-items:center;justify-content:center;border-radius:.9rem;background:rgba(0,82,204,.1);font-size:1.8rem;line-height:1;color:rgb(var(--primary))}
.org-node-btn{border-radius:.6rem;background:rgb(var(--surface-container-high));padding:.35rem .6rem;font-size:.75rem;font-weight:900;color:rgb(var(--primary))}
