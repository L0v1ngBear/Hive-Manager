...
# Flow: preflight -> clean-launch gate -> backup -> versioned migrations -> schema verification.
cd "$(dirname "$0")/.."
...
require_executable "db-migrations/scripts/check-order-multi-shipment-clean-launch.sh"
...
echo "0/5 Verify managed database state..."
ALLOW_FRESH_DATABASE=NO DATABASE_NAME="${DATABASE_NAME}" bash db-migrations/scripts/check-database-state.sh

echo "1/5 Run database preflight..."
DATABASE_NAME="${DATABASE_NAME}" bash db-migrations/scripts/preflight-online.sh

echo "2/5 Enforce order multi-shipment clean-launch gate..."
DEPLOY_DIR="$(pwd)" DATABASE_NAME="${DATABASE_NAME}" bash db-migrations/scripts/check-order-multi-shipment-clean-launch.sh

echo "3/5 Backup before migration..."
