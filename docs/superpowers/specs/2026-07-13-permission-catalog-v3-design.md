...
- 内置角色矩阵对新租户和已有租户保持一致，角色目录升级后可安全同步。
...
员工权限页面中的复选框表示“最终是否有效”：

- 角色未授予时勾选：写入个人 `GRANT`。
- 角色已授予且没有覆盖时取消：写入个人 `DENY`。
- 个人 `DENY` 后重新勾选：删除 `DENY`，恢复角色授权。
- 个人 `GRANT` 后取消：删除 `GRANT`，恢复角色未授权状态。
- 点击“恢复角色默认”：删除该节点的个人覆盖。

父节点支持“全部允许”“全部禁止”“恢复角色默认”，服务端只展开其下启用的可分配叶节点。批量操作必须在提交前显示受影响叶节点数量。
...
终态或不支持某动作的状态不创建对应动作叶节点。例如 `budget-completed`、`cancelled` 不创建推进节点。旧 `order:status:<status>` 授权只迁移为新的 `:view`，不会自动扩大为推进、回退或取消权限。
...
由于系统尚未上线，现有测试订单不做模糊推断。重置后的新数据按上述规则生成；保留的演示订单以创建人回填销售责任，无法确定生产责任的订单由企业负责人首次流转时指定。
...
保留并重建：`EMPLOYEE`、`QUALITY_STAFF`、`QUALITY_MANAGER`、`HR_STAFF`、`HR_MANAGER`、`DOCUMENT_MANAGER`、`EQUIPMENT_STAFF`、`EQUIPMENT_MANAGER`。旧 `TENANT_OWNER` 合并到 `ADMIN`，旧 `AI_MANAGER` 和 AI 权限彻底退役。自定义角色保留，但只迁移到语义明确的新叶节点。
...
- 每个叶节点显示最终复选框、角色来源标签、个人覆盖标签和说明。
- 角色授权显示“来自：销售专员”等来源；多个角色全部列出。
- 个人追加显示“个人增加”；个人禁止显示“个人禁用”。
- 父节点只显示全选、半选、未选计算结果，不作为权限保存。
- 保存前显示增加、禁用、恢复角色默认的数量摘要。

页面重新打开时，已有的最终权限必须正确打勾；个人禁用的角色权限不打勾，并明确显示“角色有权限，个人已禁用”。

### 8.2 API

```http
GET /employee/{userId}/permission-profile
PUT /employee/{userId}/permission-overrides
```
...
服务端在事务内锁定目标员工覆盖记录，校验版本、租户、员工状态和叶节点可分配性，再整体替换该员工覆盖。版本冲突返回 `409`，前端提示权限已被其他管理员修改并重新加载，不覆盖新数据。
...
1. 备份权限、用户、角色和订单责任相关表。
2. 扩展权限目录和用户版本字段，建立新节点但暂不删除旧节点。
3. 建立明确的旧码到新码映射；同一目标同时出现 `GRANT`、`DENY` 时按 `DENY` 优先。
4. 重建全部内置角色矩阵，不再跳过已存在的内置角色。
5. 自定义角色和个人覆盖只迁移语义明确的叶节点；旧订单状态权限只映射到 `:view`。
6. 新建订单责任表并回填可确定的销售责任。
7. 运行完整计数、孤儿关系、角色矩阵 checksum 和权限冲突校验。
8. 校验通过后软删除旧通配符、空销售/生产根、AI 权限和旧同义权限。
9. 递增权限目录版本并切换双端缓存命名空间。

历史迁移不能修改。回滚使用更高版本逆向迁移或数据库备份，不通过恢复旧迁移文件实现。
...
- 所有角色权限变更和员工个人覆盖变更记录操作者、目标用户、变更前后、原因、时间和租户。
...
2. 员工个人权限页只使用一棵树，并能准确解释每个最终勾选状态。
...
10. 新租户和已有租户的内置角色矩阵一致，自定义角色及可明确映射的个人覆盖保留。
11. 历史迁移 SHA-256 不变，新迁移通过空库和升级库演练。
