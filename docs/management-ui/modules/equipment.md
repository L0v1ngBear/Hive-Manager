...
- 新建、编辑、停用、导出分别按 `equipment:create`、`equipment:update`、`equipment:disable`、`equipment:export` 保持可见但禁用，并用 tooltip 说明原因。
...
- 后端按 V3 拆分写入权限：新建要求 `equipment:create`，编辑要求 `equipment:update`，停用要求 `equipment:disable`，导出要求 `equipment:export`。
- 新建、编辑、停用、保存和导出命令按各自精确权限保持可见；无权时按钮禁用，tooltip 说明原因，处理函数也会拒绝调用。
- 无 `equipment:list` 时页面在本地呈现 forbidden 授权态且不请求设备列表；详情/巡检权限不会隐式扩大列表权限。
...
- 路由允许仅有详情或巡检权限的角色进入，但当前没有合法设备 ID 参数或上下文；页面不会请求无权列表，因此此类角色只能看到本地授权态，需后端/路由另行提供受权设备上下文后才能直接访问详情或巡检。
...
- [x] 源码契约测试验证缺少对应 V3 设备动作权限时，新建、编辑、停用、保存和导出保持可见、禁用并显示原因。
- [ ] 关键字、状态、分页和空列表正确。
