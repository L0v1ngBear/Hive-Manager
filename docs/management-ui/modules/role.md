...
## 2026-09-19 权限保存遇失效员工绑定（热修）

- 现象：某角色点"确认分配"后先弹后端「员工不存在」，再弹「保存失败，请检查网络环境」，权限未保存；同租户其他角色正常。
- 根因：`RoleService#evictRoleUsersPermissionCache` 刷新绑定员工权限版本时，若 `UPDATE user ... WHERE tenant_code=? AND id=?` 影响行数为 0（该角色存在状态有效、但用户在 `user` 表已缺失或租户编码不一致的绑定），原实现直接抛 `BusinessException(404,"员工不存在")`，导致整次 `@Transactional` 授权保存回滚；前端 catch 又用网络文案兜底，掩盖真实原因。
- 修复：影响行数 ≠ 1 时跳过并打 WARN（`角色权限变更时跳过失效的员工绑定: tenantCode=…, roleId=…, userId=…`），只对真实存在的员工轮换权限版本与清缓存；前端仅在真正网络异常时提示网络问题。
- 边界：本次不清理失效绑定数据，先观察日志。`sys_user_role` 无外键、写入时不校验用户存在，撤销绑定又是租户维度（`markActiveRolesDeleted` 按 `tenant_code + user_id`），因此租户编码分叉留下的旧绑定无法被应用清理。
- 验证：新增回归用例（角色同时存在失效绑定与正常绑定时保存成功、只清正常绑定缓存）；后端全量 477 项通过；真实浏览器点验待发布后执行。
