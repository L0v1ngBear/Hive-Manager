...
- 快捷入口、摘要、公告、重要公告、业务提醒和考勤卡片按 30rem、48rem、68rem、72rem 四级内容宽度在 1/2/3 列之间重排。
- 公告区在平板宽度变为双列，主业务区只在内容区达到 68rem 后进入左右双栏；宽度不足时按自然阅读顺序单列展示。
- 卡片、标题和操作区允许收缩与换行，主体最大宽度为 `max-w-7xl`。
