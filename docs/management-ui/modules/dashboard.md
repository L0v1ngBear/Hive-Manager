...
- “本月新增订单”使用白色卡片，点击后进入订单列表并带入本月起止日期；“预警订单”点击后进入订单列表并启用未更新预警筛选。
...
6. 两个订单摘要入口使用结构化路由对象：本月订单传递 `createStart/createEnd`，预警订单传递 `staleOnly=1`；订单页统一读取这些参数并刷新列表。
...
- 页面以 `.dashboard-overview` 的实际内容宽度作为容器查询基准，不受左侧栏展开、收起或浏览器非全屏状态干扰。
- 快捷入口、摘要、公告、重要公告、业务提醒和考勤卡片按 30rem、48rem、68rem、72rem、88rem 五级内容宽度在 1/2/3 列之间重排。
- 公告区在平板宽度变为双列；主业务区达到 68rem 后将企业公告与重要公告并排，并让业务提醒、考勤卡片占满下一行；宽度不足时按自然阅读顺序单列展示。
- 卡片、标题和操作区允许收缩与换行；主体使用父级提供的完整内容宽度，不再受 `max-w-7xl`/80rem 固定上限限制。
- 公告和提醒列表使用隐藏滚动条的局部滚动容器。

## 保留项

- overview 数据映射、快捷动作 route、公告 API 参数和发布权限未改变。
- 摘要卡保持当前信息密度和三列响应式布局；本月订单、预警订单为可键盘聚焦的按钮，待我审批仍为只读统计。
...
- [ ] 左侧栏展开、收起以及浏览器非全屏时，布局按内容区宽度重排且不出现横向滚动。
