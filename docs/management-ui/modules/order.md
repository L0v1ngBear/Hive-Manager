...
- 查看订单明细、生产地点、多个附件、多条物流、开票、流转码和状态流转日志。
- 新建订单；客户与项目可从客户选项联动，也允许录入新值。
- 新建或编辑订单时可从“北京”“海宁分公司”中选择生产地点；未选择时保持为空，兼容上线前已经存在的订单。
- 普通订单从“待确认”推进到“待收款”前，弹窗显示当前生产地点并要求再次确认；未选择时不保存、不推进，取消确认时返回表单继续修改。
- 编辑订单主体、明细、生产地点、状态、物流子表、开票、业务时间和附件集合。
- 订单备注使用独立多记录：可新增、可修改，不允许删除；每条显示最后修改人和最后修改时间。
- 按当前订单阶段权限执行推进、回退审批和流转码补打。
- 配置分订单小项的未更新预警天数，并支持全量或单条重新计算；全量“重新更新预警”执行前必须二次确认，取消时不发起刷新请求。
...
- 订单领域只调用 `LogisticsTrackingGateway` 统一入口；Gateway 根据 `LOGISTICS_PROVIDER` 分派供应商子实现。默认实现为 `aliyun-market`，业务服务不直接拼接供应商请求参数；旧 APISpace 实现继续由显式配置保留。
- 阿里云市场查询使用 `expressNo`、可选客户电话后四位 `mobile` 与可选快递编码 `cpCode`；未映射的快递公司不传编码并由供应商自动识别。AppCode 仅存在服务器环境变量中。APISpace 查询仍只发送客户电话数字的最后四位，Token 同样仅存在服务器环境变量中。
...
- `productionLocation` 仅接受“北京”或“海宁分公司”，允许为空以兼容既有订单；只在新建/编辑表单和订单详情中使用，不加入列表、筛选、统计或导出。
...
- [ ] 新建订单可选择北京或海宁分公司，保存后详情正确显示；旧订单空值显示“未设置”，列表、筛选、统计和导出不增加该字段。
- [ ] 待确认订单推进时准确显示已选生产地点；未选择或取消二次确认时不调用保存和推进接口，其他状态推进不弹生产地点确认。
- [ ] “重新更新预警”确认后才调用全量刷新接口，取消或关闭确认框时不调用接口；单条预警刷新行为保持不变。
- [ ] 动态列顺序刷新后保留，恢复默认可用。
