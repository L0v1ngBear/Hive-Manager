...
| GET  | `/document/breadcrumbs?documentId=` | 当前目录面包屑                    | `document:list`          |
...
- 面包屑与目录列表统一使用 V3 的 `document:list`；两个根目录入口、上级、面包屑和双击文件夹入口检查同一个精确权限。
- 当前页导出检查 `table:export`；目录导航检查 `document:list`。命令保持可见，权限不足时置灰、禁用鼠标并通过 title 说明原因。
...
- 缺少 breadcrumbs 权限时，两个根目录入口、上级、面包屑与文件夹导航保持可见但置灰并显示原因，事件边界不会调用目录 API。
