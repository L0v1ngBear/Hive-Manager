...
- 审批中心确认“待确认”的销售订单时，快捷确认和详情确认都会显示订单生产地点并要求二次确认；地点为空或取消时不提交审批。生产订单及其他审批类型不受该确认规则影响。
- 配置五类审批的默认负责人，并为每类选择“全部通过（AND）”或“一人通过即可（OR）”。
- 新建财务申请，选择审核人并上传、下载附件。
- 新建离职申请，填写预计离职日期、原因、交接说明和审核人。
- 请假、财务、离职列表固定使用 `related` 范围：申请人是当前用户、当前审批人包含当前用户，或审批候选历史中存在当前用户，均会展示；已处理及已流转给下一负责人的相关历史记录不会消失。
- 展示“我的申请”“待我审批”“已通过”等跨请假、财务、离职标签的服务端汇总统计，不再用当前标签页的有限列表临时计数；订单和质量仍沿用各自领域的待审批统计。
...
7. 财务/离职提交生成审批编号，成功后切换相应标签并同时刷新列表、汇总看板和侧栏待办红点。
8. 默认审核人保存后刷新配置；具体申请仍可提交显式审核人集合。
9. 多审核人审批写入 `approval_auditor_candidate`，并把提交时的 `AND/OR` 规则固化到候选集合；服务在同一事务内锁定候选、原子记录当前审核人的决定，再返回 `PENDING/APPROVED/REJECTED`。`AND` 要求全部通过且任一驳回即结束；`OR` 在任一人通过时结束，只有全部驳回才拒绝。历史配置和已在途审批缺少规则时按 `AND` 兼容。
...
12. 订单审批接口返回本次决定、流程是否完成、剩余待审批人数和准确提示。单个审核人处理成功但候选集合仍为 `PENDING` 时，只提示“本次审批已提交，仍有 N 位审批人待处理”；只有全部通过后才提示审批已全部通过，驳回时明确提示订单审批已驳回。操作日志使用“处理订单审批”的中性描述，不把单人处理成功误记为整个订单已确认。
...
- 汇总看板同样使用递增 request id；审批或提交成功后立即刷新，侧栏红点监听审批变更、窗口聚焦和路由切换，并以 30 秒轮询兜底。
...
- [ ] 分别验证 `AND` 的“全部通过/任一驳回”和 `OR` 的“任一通过/全部驳回”，并确认修改默认规则不会改变已提交审批。
- [ ] 对同一审批连续提交、双击并发和两浏览器并发，验证幂等与副作用只执行一次。
- [ ] 并发提交同一离职申请，验证最多一张待审批单。
- [ ] 验证请假、离职、订单、质量通过/拒绝后的关联领域状态。
- [ ] 多审核人订单由首位审核人通过后，验证页面显示剩余待审批人数且不提示订单已确认；最后一位通过后再显示全部通过，驳回提示不得复用通过文案。
